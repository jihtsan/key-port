...
@MainActor final class AccessPilotTests: XCTestCase {
    ...
    private func key(_ store: AccessPilotStore) throws -> SSHKeyRecord {
    ...
    func testEnrollmentReconcilesInstallsVerifiesAndPersistsWithoutPassword() async throws {
        ...
        let stateURL = store!.paths.applicationSupport.appendingPathComponent("access-pilot-v1.json")
        ...
        let restored = try AccessPilotStore(home: root)
    ...
    func testNewAddressKeepsNodeAndSkipsInstallationWhenKeyWorks() async throws {
        let store = try AccessPilotStore(home: directory()); let key = try key(store)
    ...
    func testExclusiveStoreAndCorruptionDoNotSilentlyResetData() throws {
        let root = try directory()
        var store: AccessPilotStore? = try AccessPilotStore(home: root)
        XCTAssertThrowsError(try AccessPilotStore(home: root))
        let url = store!.paths.applicationSupport.appendingPathComponent("access-pilot-v1.json")
        store = nil
    ...
    func testConfigurationRejectsExpansionAndInjection() {
        for value in ["x\nProxyCommand evil", "x\"", "%h", "x\\y", "\0"] { XCTAssertFalse(AccessPilotStore.safeConfigValue(value)) }
        XCTAssertEqual(AccessPilotStore.shellQuote("a'b"), "'a'\\''b'")
    }
...
private actor PilotExecutor: ProcessExecuting {
    ...
    func execute(_ request: ProcessExecutionRequest) async throws -> ProcessExecutionResult {
        ...
        guard !results.isEmpty else { throw AccessPilotError.configuration }
