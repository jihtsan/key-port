...
在服务器详情点击「添加地址」，或打开「添加或导入服务器」，在地址字段下选择「检测并导入 Tailscale 地址」。KeyPort 读取本机 Tailscale 客户端状态，显示设备的 MagicDNS、IPv4、IPv6 与本次检测到的在线状态；支持按设备名或地址搜索。可勾选同一设备的多个地址，或点击「全选地址」后「导入所选地址」。通过「管理地址」可继续手动添加、移除或选择本次先验证的地址。填写一次 SSH 账户与端口，保存为同一服务器下的多条路径；本次只验证第一条地址，其他新增路径显示待验证，可从服务器详情逐条配置验证。
