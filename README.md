...
1. 添加或选择节点，并为节点维护局域网、Tailnet、公网或 VPN 等访问端点。
2. 在节点的“SSH 用户”中添加用户名；账户标签和密码属于用户，密码可选保存在 Keychain。
3. 选择“配置访问与免密”，选定 SSH 用户和网络路径，并设置全局唯一的 SSH 别名；也可以粘贴 `ssh user@host` 自动填写。
4. 检查扫描到的 Host Key 指纹，确认服务器身份。不要在指纹异常时继续授权。
5. 在“密钥”中扫描已有身份、导入 OpenSSH 私钥，或为当前 Mac 生成 Ed25519 密钥。
6. 选择“保存并启用免密”。KeyPort 会按所选网络路径验证账户密码、安装本机公钥并复验免密 SSH。
7. 使用保存的别名连接：
...
8. 在“设备”中查看各 Mac 的密钥，在服务器详情中刷新或撤销远端授权；在“审计日志”中检查操作结果。

SSH 用户是节点下的账户身份，密码与密钥授权按账户共享；SSH 别名属于“账户 + 网络路径”的连接配置。同一用户可以为 Tailnet、局域网等路径保存多个不同别名，而不会重复创建账户或授权。
...
CloudKit 上传前会移除本地私钥路径、SSH Agent 状态、当前设备标记、Tailscale 本机观测、检测状态和审计日志。统一拓扑只同步 Node、Endpoint、Service、SSH 账户和 Tailscale 稳定身份等非敏感元数据。加密 `.keyport` 归档包含服务器、别名、公钥、设备和授权元数据，但不包含服务器密码、私钥、本地路径或审计日志。
