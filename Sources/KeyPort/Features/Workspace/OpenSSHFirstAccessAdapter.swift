...
@MainActor final class OpenSSHFirstAccessAdapter: FirstAccessAdapter {
    ...
    private var observed: WorkspaceStore.Trust?
    ...
    init(store: WorkspaceStore, expectedServerID: String? = nil, executor: any ProcessExecuting = ProcessExecutor(), askPassPath: String? = nil) {
    ...
    func inspectHost(address: String, port: String) async throws -> AccessHostIdentity {
        ...
        guard draft.validationMessage == nil, WorkspaceStore.safeConfigValue(address), let number = Int(port) else { throw AccessFlowFailure.unreachable }
        ...
        let endpointTrust = store.state.trusts.first { $0.address == address && $0.port == number && $0.key.algorithm == hostKey.algorithm }
        let serverID = expectedServerID ?? endpointTrust?.serverID ?? UUID().uuidString
        let nodeTrusts = store.state.trusts.filter { $0.serverID == serverID && $0.key.algorithm == hostKey.algorithm }
    ...
    func login(draft input: AccessFormDraft, identity: AccessHostIdentity) async throws {
        ...
        guard let observed, identity.serverID == observed.serverID,
              ...
              validationInput.validationMessage(in: store.aliases) == nil, WorkspaceStore.safeConfigValue(input.account),
        ...
        if draft.existingKey && !draft.privateKeyPath.isEmpty {
            ...
            guard path.hasPrefix("/"), WorkspaceStore.safeConfigValue(path), FileManager.default.fileExists(atPath: path),
                  let line = try? String(contentsOfFile: path + ".pub", encoding: .utf8), let parsed = PublicKeyParser.parse(line) else { throw WorkspaceError.missingKey }
            ...
            try store.addKey(selectedKey)
                  ...
                  let existing = store.state.keys.first(where: { $0.id == keyID && $0.deviceID == deviceID && $0.privateKeyPath != nil }) { selectedKey = existing }
        else if let existing = store.state.keys.first(where: { $0.origin == .generated && $0.deviceID == deviceID && $0.privateKeyPath != nil }) { selectedKey = existing }
        else {
            guard !draft.existingKey else { throw WorkspaceError.missingKey }
            selectedKey = try await keyService.generate(device: .init(id: deviceID, name: "KeyPort", isCurrent: true))
    ...
    func openTerminal(command: String) async throws {
        guard !command.isEmpty, command == handoffCommand else { throw WorkspaceError.terminal }
    ...
    static func handoff(_ command: String, directory: URL) async throws {
        guard let terminal = NSWorkspace.shared.urlForApplication(withBundleIdentifier: "com.apple.Terminal") else { throw WorkspaceError.terminal }
...
extension ManagedAliasInstallation.Failure: AccessFlowFailureDetailProviding {
