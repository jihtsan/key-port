...
阶段结论：**本次命名界面功能与视觉已验收；前置 #90/#92 的视觉例外仍待确认。保持堆叠草稿，不合并，不关闭 #78/#90/#92/#94。**

2026-09-14 用户在实际验收询问中答复：“通过本次命名界面视觉验收”。该确认覆盖本次主页别名/中文描述层级、表单和成功页的原生字形与尺寸观感；不把工具缩图升级为原尺度像素证明。此前 #90 的主页原尺度/系统字形、#92 的新增失败/取消页等视觉例外没有因此获得确认。
...
| 设计节点 | 实现与差异 | 结论 |
|---|---|---|
| 26:2 表单 | 880×740，外边距 32、面板 816×464；主内容间距 10，行高 48、输入高 40，标签宽 120/间距 20。别名、规则、描述、地址/端口、账户、密码依次排列；删除名称与高级别名入口 | 已验收（结构/交互自验及本次用户视觉确认） |
...
| 27:2 主页 | 别名主标题、中文描述副标题；mac-studio 无描述时无副标题；搜索提示与匹配覆盖两字段；连接设置摘要改为描述、地址策略 | 本次命名层级与原生观感已验收；前置主页原尺度证明仍保留边界 |
...
| 预览说明 | 页脚保留隔离演示和手动场景控制，成功加“模拟”；不显示“待确认设计”历史文案 | 本次模拟边界与命名界面观感已验收 |
...
| 本次命名界面原生观感 | 已验收（用户） | 2026-09-14 用户明确答复通过；不等于前置整体原尺度/新增状态例外通过 |
...
3. 本次命名界面视觉已有上述实际答复。#90/#92 遗留的整体原尺度/新增错误与取消页视觉例外仍待单独确认；不以本次答复推定这些项通过，不合并前置或本 PR。
