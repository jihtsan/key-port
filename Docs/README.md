...
当前正式运行入口、工作区与迁移边界请先阅读 [README](../README.md) 和 [Issue #105 验收](Issue105/acceptance.md)。下列文档保留历史设计与阶段证据，不表示旧入口仍在运行。

## 本期产品范围与整合规划

- [统一 SSH 连接策略实施方案](./统一SSH连接策略实施方案.md)：2026-09-15 基于当前正式入口核查的恢复方案；统一别名、多地址策略、OpenSSH 执行、信任映射、迁移同步与阶段验收。待实施。

- [一期产品边界与功能整合规划](./一期产品边界与功能整合规划.md)：2026-09-11 确认的管理应用、本机 OpenSSH、辅助脚本边界；删除/合并/下沉清单、优化优先级、分期与验收标准。属于规划，尚未实施功能整合。
- 本期产品范围优先阅读上述规划；下文网络拓扑及协议扩展文档仍提供领域与历史设计依据，既有安全和迁移契约继续有效。
...
- 想理解本期方向：一期产品边界与功能整合规划 → 领域词汇 → 网络拓扑领域模型重构设计 → ADR 0002 → Graph 架构 → 整体重构计划。
