...
- 文档版本：V1.0
- 日期：2026-08-29
- 关联 Issue：[GitHub #33](https://github.com/jihtsan/key-port/issues/33)
...
> 本文是全量重构的目标模型，不是对当前 `HostV6` 的增量修补。现有 [Graph 架构文档](./Graph拓扑与跨设备授权架构.md) 和 [整体重构计划](./整体重构与迁移计划.md) 中以 Host v6 为中心的实体关系，保留为当前实现和迁移证据；涉及实体边界时，以本文为准。
...
| 当前概念 | 目标概念 | 处理方向 |
| --- | --- | --- |
| `HostV6.Host` | `Node` | 删除 Host 作为根身份；保留稳定机器事实和角色 |
...
| V5 `ServerConnection` / `AppSnapshot` | 迁移输入 | 新模型稳定后删除运行时写路径和兼容领域对象 |

### 明确可删除或不再作为独立实体的内容

- `Host` 与 `Device` 两套机器根身份。
- 以 `host + port` 生成机器 ID 的逻辑；Endpoint 变化不能改变 Node 身份。
- 持久化的 `Host Ping`、`HostStatus` 或把一次状态提升为 Node 永久属性的字段。
- V5 `ServerConnection` 与 V6 Host 并行存在的运行时写模型。
...
1. 先把每个旧连接导入为 Node candidate 和 Endpoint candidate，保留原始 ID、来源和 endpoint key。
2. 如果旧数据明确表示多个地址属于同一主机，则归并到同一个 Node，并分别生成 Endpoint。
3. 如果只有名称、IP 或端口相似，则保留为多个候选并请求用户确认，不静默合并。
4. 完成 Node 归并后，再迁移 SSH Account、SSH Authorization、Host Key Trust、Service 和本地观测的引用。
5. 新模型稳定后删除旧 V5/V6 运行时写路径；只保留必要的导入和恢复工具。

当前 `HostV6.StableID.host(legacyEndpointKey:)` 的 endpoint 派生策略不应迁移为目标规则。它最多只能作为旧数据导入的 provenance，不能继续负责新 Node 的身份生成。

## 9. Graph 的新投影边界

Graph 的默认画布只显示 Node 和 Service；Endpoint、SSH Account、SSH Key、Host Key Trust 和观测在 Inspector 或边详情中展开。
...
“两台机器都能 Ping 通”“名字相似”“在同一网段”都不能单独生成访问边。当前代码中的 [GraphWorkspaceModel](../Sources/KeyPort/Stores/GraphWorkspaceModel.swift:7) 和 [TopologyGraphProjector](../Sources/KeyPortCore/Hosts/TopologyGraphProjector.swift:6) 可以作为投影职责的参考，但其 `device/host` 节点类型、旧关系链和固定布局需要在新模型阶段重写。
...
1. **领域基线**：采用本文和 `CONTEXT.md` 的术语，停止新增 Host/Device 平行概念。
2. **核心模型**：建立 Node、Role、Workspace Device Profile、Endpoint、Service 和协议访问关系。
3. **SSH 第一阶段**：迁移 SSH Account、SSH Key、SSH Authorization、SSH Host Key Trust 和本地验证；只支持 SSH。
4. **导入与数据迁移**：从旧 Host/Device/ServerConnection 导入，处理多 Endpoint 和合并候选。
5. **Graph 投影**：用 Node/Service 作为主要画布节点，地址、账户、信任和观测进入 Inspector。
6. **删除旧模型**：新读写路径稳定后删除 V5 写模型、旧 Host/Device 根实体、endpoint 派生 Host ID 和重复编排。
